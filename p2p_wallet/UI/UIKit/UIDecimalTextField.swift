    ...
    public func textField(
        ...
        if string.isEmpty {
            DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(100)) { [unowned self] in
                forwardedDelegate?.decimalTextFieldDidReceiveValue(
                    self,
                    value: text?.toDouble(decimalSeparator: decimalSeparator)
                )
            }
        ...
        let isANumber = formatter.number(from: updatedText.replacingOccurrences(
            of: decimalSeparator,
        ...
        if updatedText.starts(with: "0") && !updatedText.starts(with: "0\(decimalSeparator)") {
            ...
            forwardedDelegate?.decimalTextFieldDidReceiveValue(
                self,
                value: updatedText.toDouble(decimalSeparator: decimalSeparator)
            )
            return false
        }
...
private extension String {
    func toDouble(decimalSeparator: String) -> Double? {
        guard let number = NumberFormatter().number(from: replacingOccurrences(
            of: decimalSeparator,
            with: Locale.current.decimalSeparator ?? "."
        ))?.doubleValue
        else {
