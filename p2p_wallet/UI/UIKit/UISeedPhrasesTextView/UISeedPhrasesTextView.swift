...
//        placeholder = L10n.enterSeedPhrasesInACorrectOrderToRecoverYourWallet
    ...
    override public func closestPosition(to _: CGPoint) -> UITextPosition? {
...
//    }
    ...
    private func handlePasting(range: NSRange, text: String) {
        ...
        // if not stand at the begining and there is no index before it
            ...
            if !hasPhraseSeparatorBeforeLocation(range.location) {
                let phraseSeparator = NSMutableAttributedString(
                    string: phraseSeparator,
                    attributes: Self.defaultTypingAttributes
                )
                addingAttributedString = addingAttributedString
    ...
    private func insertOnlyLetters(range: NSRange, text: String) {
        ...
        if allowedCharacters.isSuperset(of: characterSet) {
            ...
            textStorage.replaceCharacters(
                in: selectedRange,
                with: NSAttributedString(string: text.lowercased(), attributes: Self.defaultTypingAttributes)
            )
    ...
    private func rearrangeIndexes() {
        ...
        for range in indexRanges {
            ...
            textStorage.replaceCharacters(
                in: .init(location: range.location + lengthDiff, length: range.length),
                with: indexAttributedString
            )
    ...
    private func rangeOfIndexBeforeLocation(_ location: Int) -> NSRange? {
        ...
        guard let result = indexRegex.matches(
            in: text,
            range: NSRange(
                location: location > maxIndexLength ? location - maxIndexLength : 0,
                length: location >= maxIndexLength ? maxIndexLength : minIndexLength
            )
        ).last
        else {
            return nil
    ...
    private func rangeOfIndexAfterCurrentSelectedLocation() -> NSRange? {
        ...
        guard let result = indexRegex.matches(
            ...
            range: NSRange(
                location: location,
                length: text.count >= location + maxIndexLength ? maxIndexLength : minIndexLength
            )
        ).first
        else {
            return nil
    ...
    private func rangeOfPhraseSeparatorBeforeLocation(_ location: Int) -> NSRange? {
        ...
        guard let result = regex.matches(
            in: text,
            range: NSRange(location: location - phraseSeparator.count, length: phraseSeparator.count)
        ).last
        else {
            return nil
        }
