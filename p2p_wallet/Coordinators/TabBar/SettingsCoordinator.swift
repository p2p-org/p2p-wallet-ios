...
final class SettingsCoordinator: Coordinator<Void> {
    ...
    override func start() -> AnyPublisher<Void, Never> {
        ...
        let settingsVC = settingsView.asViewController() as! UIHostingControllerWithoutNavigation<SettingsView>
        navigationController.setViewControllers([settingsVC], animated: false)

        settingsVC.viewWillAppear
            .sink(receiveValue: { [unowned settingsVC] in
                settingsVC.navigationIsHidden = true
            })
            .store(in: &subscriptions)
        settingsVC.viewWillDisappear
            .sink(receiveValue: { [unowned settingsVC] in
                settingsVC.navigationIsHidden = false
            })
            .store(in: &subscriptions)
