...
public actor StateMachine<S: State> {
    ...
    public func accept(event: S.Event) async throws -> S {
        do {
            let state = try await currentState.accept(
                currentState: stateSubject.value,
                event: event,
                provider: provider
            )
...
public extension HierarchyStateMachine {
    static func <- (lhs: HierarchyStateMachine, rhs: S.Event) async throws {
        try await lhs.accept(event: rhs)
    }
}
