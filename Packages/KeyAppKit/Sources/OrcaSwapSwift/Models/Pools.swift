...
public extension PoolsPair {
    func constructExchange(
        ...
        if count == 1 {
            ...
            return (.init(
                ...
                cleanupInstructions: pool0AccountInstructions.cleanupInstructions + pool1AccountInstructions
                    .cleanupInstructions,
    ...
    func getInputAmount(
        ...
        // direct
        if count == 1 {
            guard let inputAmount = try? pool0.getInputAmount(
                minimumReceiveAmount: minimumAmountOut,
                slippage: slippage
            )
            else { return nil }
            return inputAmount
            ...
            let pool1 = self[1]
            guard let inputAmountPool1 = try? pool1.getInputAmount(
                minimumReceiveAmount: minimumAmountOut,
                slippage: slippage
            ),
                let inputAmountPool0 = try? pool0.getInputAmount(
                    minimumReceiveAmount: inputAmountPool1,
                    slippage: slippage
                )
            else { return nil }
    ...
    func getMinimumAmountOut(
        ...
        // transitive
            ...
            let pool1 = self[1]
            guard let minimumAmountOut = try? pool1.getMinimumAmountOut(
                inputAmount: outputAmountOfPool0,
                slippage: slippage
            )
            else { return nil }
            return minimumAmountOut
