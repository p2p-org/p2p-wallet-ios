...
//        ]
    func testGetTradablePoolsPairs() async throws {
        ...
        let btcSOLAquafarm = pools
            .first(where: { $0.contains(where: { $0.account == "7N2AEJ98qBs4PwEwZ6k5pj8uZBKMkZrKZeiC7A64B47u" }) })!
            .first!
        ...
        let ethSOL = pools
            .first(where: { $0.contains(where: { $0.account == "4vWJYxLx9F7WPQeeYzg9cxhDeaPjwruZXCffaSknWFxy" }) })!
        ...
        let ethSOLAquafarm = pools
            .first(where: { $0.contains(where: { $0.account == "EuK3xDa4rWuHeMQCBsHf1ETZNiEQb5C476oE9u9kp8Ji" }) })!
            .last! // reversed to SOL/ETH
        XCTAssertEqual(ethSOLAquafarm.tokenAccountA,
                       "5pUTGvN2AA2BEzBDU4CNDh3LHER15WS6J8oJf5XeZFD8") // originalTokenAccountB
        XCTAssertEqual(ethSOLAquafarm.tokenAccountB,
                       "7F2cLdio3i6CCJaypj9VfNDPW2DwT3vkDmZJDEfmxu6A") // originalTokenAccountA
    ...
    func testGetBestPoolsPairForInputAmount() async throws {
        ...
        let inputAmount: UInt64 = 100_000 // 0.1 BTC
        ...
        XCTAssertEqual(estimatedAmount, 1_588_996) // 1.588996 ETH
    }
    ...
    func testGetBestPoolsPairForInputAmountWithSmallAmount() async throws {
        ...
        let bestPoolsPair = try orcaSwap.findBestPoolsPairForInputAmount(
            inputAmount,
            from: poolsPairs,
            prefersDirectSwap: true
        )
        let estimatedAmount = bestPoolsPair?.getOutputAmount(fromInputAmount: inputAmount)
    ...
    func testGetBestPoolsPairForEtimatedAmount() async throws {
        ...
        let estimatedAmount: UInt64 = 1_600_000
        let poolsPairs = try await orcaSwap.getTradablePoolsPairs(fromMint: btcMint, toMint: ethMint)
        ...
        XCTAssertEqual(inputAmount, 100_697) // 0.100697 BTC
    }
...
//        ]
    func testGetTradablePoolsPairsReversed() async throws {
        let poolsPair = try await orcaSwap.getTradablePoolsPairs(fromMint: socnMint, toMint: btcMint)
            .sorted(by: { $0.first!.account < $1.first!.account }).first!
    ...
    func testGetBestPoolsPairReversed() async throws {
        ...
        XCTAssertEqual(inputAmount2, 413_909_257_520) // 413.909257520 SOCN
    }
}

private class MockSolanaAPIClient: JSONRPCAPIClient {
    override func getTokenAccountBalance(pubkey: String,
                                         commitment _: Commitment?) async throws -> TokenAccountBalance
    {
        ...
        if pubkey == "5pUTGvN2AA2BEzBDU4CNDh3LHER15WS6J8oJf5XeZFD8" {
            return .init(amount: 103_486.885774058, decimals: 9)
        }
        ...
        if pubkey == "HMFLg2GtbWSSEe92Vuf2LQdUpCacGj2m2PwvMqzwQFNi" {
            return .init(amount: 1_022_524.636749, decimals: 6)
        }
        ...
        if pubkey == "JA98RXv2VdxQD8pRQq4dzJ1Bp4nH8nokCGmxvPWKJ3hx" {
            return .init(amount: 1_350_604.603948, decimals: 6)
        }
        ...
        if pubkey == "FZFJK64Fk1t619zmVPqCx8Uy29zJ3WuvjWitCQuxXRo3" {
            return .init(amount: 300_343.554305, decimals: 6)
        }
        ...
        if pubkey == "G45yhM5mZ5RXZpLxGWLk3PVzdAp33z8aH6F9mLW8fQj3" {
            return .init(amount: 1_259_503.315676, decimals: 6)
        }
        ...
        if pubkey == "5HaG31FQS4McBVcHxVfwaKaWXE3VCGqvJ1ZDkTxs94cQ" {
            return .init(amount: 360_430.732774, decimals: 6)
        }
        ...
        if pubkey == "6uUn2okWk5v4x9Gc4n2LLGHtWoa9tmizHq1363dW7t9W" {
            return .init(amount: 159_976.903632, decimals: 6)
        }

        if pubkey == "Ajf4bxNoKCyFVfV35sRTgGwZK1dfJJJVXgNFs7ncC5EF" {
            return .init(amount: 929_618.189202, decimals: 6)
        }
        ...
        if pubkey == "AiwmnLy7xPT28dqZpkRm6i1ZGwELUCzCsuN92v4JkSeU" {
            return .init(amount: 164_720.899813, decimals: 6)
        }
        ...
        if pubkey == "75HgnSvXbWKZBpZHveX68ZzAhDqMzNDS29X6BGLtxMo1" {
            return .init(amount: 2_028_563.343853, decimals: 6)
        }
