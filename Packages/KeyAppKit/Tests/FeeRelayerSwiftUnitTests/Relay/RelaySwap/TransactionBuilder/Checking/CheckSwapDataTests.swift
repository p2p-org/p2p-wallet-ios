...
final class CheckSwapDataTests: XCTestCase {
    ...
    func testCheckDirectSwapData() async throws {
        ...
        XCTAssertEqual(
            swapInstruction.keys[3],
            .writable(publicKey: "4Vfs3NZ1Bo8agrfBJhMFdesso8tBWyUZAPBGMoWHuNRU", isSigner: false)
        )
        XCTAssertEqual(
            swapInstruction.keys[4],
            .writable(publicKey: Pool.btcETH.tokenAccountA.publicKey, isSigner: false)
        )
        XCTAssertEqual(
            swapInstruction.keys[5],
    ...
    func testCheckTransitiveSwapData() async throws {
        ...
        if needsCreateTransitTokenAccount {
            ...
            XCTAssertEqual(
                createTransitTokenAccountInstruction.keys[0],
                .writable(publicKey: .btcTransitTokenAccountAddress, isSigner: false)
            )
            XCTAssertEqual(
                createTransitTokenAccountInstruction.keys[1],
            ...
            XCTAssertEqual(
                createTransitTokenAccountInstruction.keys[3],
                .readonly(publicKey: .feePayerAddress, isSigner: true)
            )
            XCTAssertEqual(
                createTransitTokenAccountInstruction.keys[4],
                .readonly(publicKey: TokenProgram.id, isSigner: false)
            )
            XCTAssertEqual(
                createTransitTokenAccountInstruction.keys[5],
                .readonly(publicKey: .sysvarRent, isSigner: false)
            )
            XCTAssertEqual(
                createTransitTokenAccountInstruction.keys[6],
        ...
        XCTAssertEqual(
            swapInstruction.keys[8],
            try .readonly(publicKey: Pool.solBTC.authority.toPublicKey(), isSigner: false)
        )
        XCTAssertEqual(
            swapInstruction.keys[9],
        ...
        XCTAssertEqual(
            swapInstruction.keys[16],
            .writable(publicKey: Pool.btcETH.tokenAccountA.publicKey, isSigner: false)
        )
        XCTAssertEqual(
            swapInstruction.keys[17],
            .writable(publicKey: Pool.btcETH.tokenAccountB.publicKey, isSigner: false)
        )
        XCTAssertEqual(
            swapInstruction.keys[18],
            .writable(publicKey: Pool.btcETH.poolTokenMint.publicKey, isSigner: false)
        )
        XCTAssertEqual(
            swapInstruction.keys[19],
        ...
        XCTAssertEqual(
            ...
            [UInt8]([4, 128, 150, 152, 0, 0, 0, 0, 0, 14, 0, 0, 0, 0, 0, 0, 0, 171, 0, 0, 0, 0, 0, 0, 0])
        )
    }
}
