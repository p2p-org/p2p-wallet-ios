...
class RelaySwapTests: RelayTests {
    ...
    private func prepareTransaction(testInfo: RelaySwapTestInfo,
                                    isTransitiveSwap: Bool?) async throws -> (
        ...
        return try await swapRelayService
            .buildSwapTransaction(
                context,
                sourceToken: .init(
                    address: PublicKey(string: testInfo.sourceAddress),
                    mint: PublicKey(string: testInfo.fromMint)
                ),
                destinationTokenMint: PublicKey(string: testInfo.toMint),
                destinationAddress: PublicKey(string: testInfo.destinationAddress),
                fee: .init(
                    address: PublicKey(string: testInfo.payingTokenAddress),
                    mint: PublicKey(string: testInfo.payingTokenMint)
                ),
                swapPools: pools,
